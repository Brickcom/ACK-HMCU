...
void ACKPlatform_WriteLightPin(E_USERGPIODEV pin, bool state)
{
    if (state)
        HAL_GPIO_OutputHigh(&g_asBoardUserGpioDev[pin]);
    else
        HAL_GPIO_OutputLow(&g_asBoardUserGpioDev[pin]);
}
